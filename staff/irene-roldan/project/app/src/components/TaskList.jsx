...
function TaskList({ boardId, columnType, updateTasks }) {
    ...
    const loadTasks = () => {
        console.log('TaskList -> loadTasks');
        try {
                ...
                try {
                    logic.deleteTask(taskId)
                        .then(() => {
                            ...
                            // Llama a la función de recarga de tareas proporcionada como prop después de eliminar una tarea
                            updateTasks();
                        })
                        .catch(error => showFeedback(error, 'error'));
                } catch (error) {
    ...
    const handleEditTask = (updatedTask) => {
        ...
        logic.editTask(updatedTask)
            .then(() => {
                loadTasks(); // Recarga las tareas después de editar una tarea
                setEditingTask(null);
            })
            .catch(error => {
    ...
    useEffect(() => {
        loadTasks();
    }, [boardId, columnType]); // Solo dispara el efecto cuando cambian `boardId` y `columnType`
